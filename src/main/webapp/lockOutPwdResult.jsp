...
<%@page import="Security.PasswordResetGrant,Security.RedirectSanitizer"%>
<%@ page contentType="text/html; charset=UTF-8" language="java" %>
<%
    response.setHeader("Cache-Control", "no-store");
    response.setHeader("Referrer-Policy", "no-referrer");
    if (!PasswordResetGrant.isPostForm(request)) { response.sendError(403, "잠금 해제에서 다시 인증해주세요."); return; }
    String resetToken = PasswordResetGrant.verify(session, PasswordResetGrant.UNLOCK,
            request.getParameter("id"), request.getParameter("verifyCode"));
    if (resetToken == null) { response.sendError(403, "인증번호를 확인하거나 다시 인증해주세요."); return; }
    String id = (String) session.getAttribute("pwdResetUserId");
%>
<!DOCTYPE html>
<html lang="ko">
<head>
...
<div class="login-container">
  ...
  <form action="resetPwd2" method="post" id="resetPwd">
	...
	<input type="text" id="userId" name="userId" value="<%=RedirectSanitizer.escapeHtmlAttribute(id)%>" class="readonly-input" readonly>
