...
| 항목 | 내용 |
|---|---|
| 형태 | 팀 프로젝트(컴퓨터공학 전공 과정) → 종료 후 개인 복구·개선 |
...
**팀 프로젝트 당시** — 팀장으로 일정과 작업 분담을 조율했고, 상품·회원·주문 기능 간 관계와 DB 테이블 설계를 맞추는 일을 맡았습니다. 기존 기능과 화면 구조는 팀 전체의 결과물입니다.

**프로젝트 종료 후 (개인)** — 버전이 서로 다른 작업본들을 분석해 하나의 실행 가능한 기준을 만들고, 다음 작업을 혼자 진행했습니다.

- 실행환경 정리(Java 21, Tomcat 9)와 DB schema 재구성
- 비밀번호·본인확인·요청 검증 등 보안 개선
- 결제 경로의 금액 위조 문제 차단
- 공개하면 안 되는 설정값·개인정보 제거
- 원본 상품 데이터와 화면 복원, 신규 설치 검증

## 주요 개선 사례

| 문제 | 해결 | 확인 |
|---|---|---|
| 작업본마다 DB 구조와 DAO 코드가 서로 맞지 않아 실행되지 않음 | 코드가 실제로 사용하는 테이블·컬럼을 기준으로 schema SQL을 다시 작성, DB 접속 정보를 환경변수로 분리 | 빈 MySQL에 공개 SQL 11개만 적용해 설치·로그인·상품·장바구니 동작 확인 |
...
- 상품 1,518개 중 314개는 원본 데이터에 사이즈 정보가 없어 장바구니에 담을 수 없습니다.
